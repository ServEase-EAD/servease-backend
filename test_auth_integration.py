...
try:
    login_response = requests.post(
        f"{base_url}/auth/login/", json=test_credentials)
    ...
    if login_response.status_code == 200:
        ...
        print(
            f"Login successful! Token length: {len(access_token) if access_token else 0}")

        headers = {"Authorization": f"Bearer {access_token}"}

        # Step 2: Test auth profile endpoint
        print("\n2. Testing auth profile endpoint...")
        auth_profile_response = requests.get(
        ...
        customer_profile_response = requests.get(
            f"{base_url}/customers/profile/", headers=headers)
        print(
            f"Customer profile status: {customer_profile_response.status_code}")
